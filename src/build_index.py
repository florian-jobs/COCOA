...
def _process_file(tableid_and_path):
    ...
    try:
        ...
        return tableid, path, len(df.columns), tmp_tokenized_df, tmp_order_index_df, None
    except Exception as e:
        return tableid, path, None, None, None, str(e)

def _commit_file(conn, tables, tableid, num_columns, tmp_tokenized_df, tmp_order_index_df):
    """
    Inserts one already-processed file's rows into mt/dt/oi/mc as a single
    transaction (see _build's docstring for why dt/mc are derived here per
...
def main(argv=None):
    """CLI entry point: parses --corpora/--limit/--workers, then (re)builds the index under a lock so concurrent builds can't collide."""
...
def _build(db_path, tables, args):
    ...
    try:
        ...
        with open(skip_log_path, "w", encoding="utf-8") as skip_log:
            ...
            def _handle_result(tableid, path, num_columns, tmp_tokenized_df, tmp_order_index_df, err):
                filename = os.path.basename(path)
                if err is not None:
                    _log_skip(filename, err)
                    return
                commit_err = _commit_file(conn, tables, tableid, num_columns, tmp_tokenized_df, tmp_order_index_df)
